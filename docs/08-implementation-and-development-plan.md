...
- **First real-model number produced (2026-07-14, local, $0):** 400 AG News items labeled by a locally-served Qwen3.5-4B via `TESSERA_ANTHROPIC_URL` (winc.cpp): 27.5% auto-labeled at 97.0% cross-validated precision — but held-back-truth validation measured 93.6% true precision (92.2% unseen), so the 95% SLA did **not** hold out-of-sample, exactly as the 0–44% bootstrap coverage CI warned. The honest-measurement layer caught its own miss. The §4.3 headline number on a frontier model and a real partner dataset is still the open Phase 0 exit item; the levers are more gold, self-consistency sampling, a cross-family judge, or a frontier labeler.
