...
**Phase 0 (Wedge) and most of Phase 1 (Trust) are built and shipped** — github.com/samdotson61/Tessera (public), 219 passing tests + CI (v0.17.0; v0.15 GitHub-native installers, v0.16 idempotency/path-portability pass, **v0.17 workflow-complete UI** — Import→Rubric→Gold→Label→Review→Export as a state-derived six-step shell with honest next-action guidance, in-browser import/rubric-editing/gold-authoring/run-with-progress/exports, tooltips on every control; built after the felt-loop principle failed its own builder, live-verified end to end as a first-time user; contract in `appmap.mp`). What actually shipped, and where it diverged from the plan below:
