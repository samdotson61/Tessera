...
**Phase 0 (Wedge) and most of Phase 1 (Trust) are built and shipped** — github.com/samdotson61/Tessera (public), 232 passing tests + CI (v0.19.0 Settings page — model auto/2B/4B/custom/stub + labeling knobs saved per-db with env-pins honest, native window by default, run auto-jumps to review; drilled in three full app sessions on real models incl. app-path parity with the measured SMS numbers to the decimal and a live engine swap from the ⚙ page; v0.18.0 zero-config auto-serve — winc's 2B/4B picked by memory (≥7GB→4B, Jobfaro rule), lazy spawn-once-reuse with exit cleanup, plan shown in the serving line, explicit config always wins; v0.17.0; v0.15 GitHub-native installers, v0.16 idempotency/path-portability pass, **v0.17 workflow-complete UI** — Import→Rubric→Gold→Label→Review→Export as a state-derived six-step shell with honest next-action guidance, in-browser import/rubric-editing/gold-authoring/run-with-progress/exports, tooltips on every control; built after the felt-loop principle failed its own builder, live-verified end to end as a first-time user; contract in `appmap.mp`). What actually shipped, and where it diverged from the plan below:
...
- **First real-model numbers produced (2026-07-14, local, $0):** 400 AG News items labeled by a locally-served Qwen3.5-4B via `TESSERA_ANTHROPIC_URL` (winc.cpp). Greedy single-sample: 27.5% coverage, 93.6% true precision (vs 97.0% CV estimate). 5-vote self-consistency: 46.8% coverage, 92.5% true (90.1% unseen) — resolution went from 5 to 48 distinct confidence values. Both miss the 95% SLA out-of-sample, as the wide bootstrap CI warned. **Structural finding:** an oracle-reviewer simulation (`scripts/simulate_review.py`) showed gold grown from the routed queue never corrects auto-region overconfidence — the docs/04 **audit sample is load-bearing**. **Audit sampling built (v0.5.0, 2026-07-17):** a deterministic `TESSERA_AUDIT_RATE` slice of auto-applied items is human-verified (label ships, verdict feeds gold + reports `audit_precision` — the production SLA check; accept confirms, edit overturns, reject un-ships and routes). Measured at 15% audit on the local run: estimate–truth gap 5.7→1.3 points, coverage honestly re-priced 46.8%→36.8%, unseen true precision 90.1%→93.9%. **Limits pass (v0.9.0, 2026-07-17):** gold at 297 makes the gate honest (95% correctly refused — the 4B's true best band is ~94%; 90% target → 64% coverage @ 94.1% TRUE, 97.2% unseen — conservative and kept; 85% → 99.2%). Low-end verdicts: 2B = economy (33.5%@90), 9B fails X1-class (~20s/item projected). Retired by measurement: cross-family ensemble (no gain, 1.5× cost), static few-shot (coverage regression; its speed premise is void — cross-item partial-prefix KV reuse nonfunctional on the current llama-server chat endpoint, upstream-worthy). Tier-0 specialist + cascade built (stdlib logreg, human-trusted training, train/calibrate split after measuring the leak); earns coverage at flywheel scale, not at 149 examples.
**Speed+accuracy pass (v0.8.0, 2026-07-17):** two features measured head-to-head on the local 4B — gold few-shot retrieval (docs/05 Phase A RAG-lite, `TESSERA_FEWSHOT`) and the **logprob head** (`TESSERA_LOGPROBS`: label-as-one-word, first-token top-logprobs = the distribution; one call/item). The logprob head dominated: 5.1 min for 400 items (3× faster than 5-vote), 64% coverage @ 92.1% true unseen (+17pts coverage, +2pts precision vs baseline), and it out-labels Haiku-4.5-with-verbalized-confidence on this task. Few-shot stacks +14pts coverage at −2pts precision (right at a 90% target). Confidence-signal quality, not model size, was the binding constraint.
**Frontier comparison run (2026-07-17, Claude Haiku 4.5, ~$1.30):** true accuracy 84% vs the local 4B's ~85% — both pinned at AG News's own label-noise ceiling. At the 95% target the gate correctly awarded Haiku ZERO coverage (gold errors inside the top confidence bands → no honest threshold), while at 90% it auto-labeled 89% of the dataset at 88.8% true (~2× the 4B's coverage, ~4pts less precise). Confirmed empirically: R3/R4 (ceiling + gold/rubric quality) dominate model choice; the corpus-inherited gold was the weak link. The §4.3 headline number now needs what it always needed: a real partner dataset with curated gold.
...
- **Automation pass (v0.10.0, 2026-07-17):** three features that close the loop into a supervised system. **Consensus gate** (`TESSERA_SPECIALIST=1`): the Tier-0 specialist joins the ensemble trained on a hash-stable half of the trusted labels, calibration uses only the other half (leak guard auto-detected from stored votes); measured on the same cached 4B calls + 297 gold — **93.5% coverage @ 92.8% TRUE (92.7% unseen) at the 90% target, vs 64% without it**; the 2B goes 33.5%@88.1% → 49.8%@98.0% TRUE (economy tier redeemed); the 95% refusal correctly stands (label noise binds, not signal). **Near-duplicate propagation** (`TESSERA_PROPAGATE`): reps + gold hit the LLM, members mirror rep label/state with provenance, stay audit-sampled, resolve as a group in review (bulk accept / group un-ship / emancipation on human touch); benchmark corpora arrive pre-deduped (~1–2% savings there) — the factor scales with real corpus redundancy. **Audit autopilot** (`TESSERA_AUTOPILOT=1`): windowed exact-binomial control from the audit verdict stream — confident breach halves allowed error (≤3 levels), clean window relaxes, inconclusive evidence accumulates; report-only re-gates never consume evidence. The docs/04 audit layer is now a closed loop: drift tightens the gate and pulls humans in instead of silently poisoning labels. Live SMS demonstration (logprob head, hidden ref; numbers corrected by the v0.12.1 erratum — the original session unknowingly ran a mute reasoning-mode model): **corrected SMS = 100% coverage @ 97.0% true with consensus (95.7% plain), both targets**; the original session's real lesson is that the safety stack contained a broken labeler nobody knew was broken — consensus routed what the dead signal couldn't support, 93 oracle-worked audits (86.0% confirmed) collapsed the broken config 100%→0.3% via gold growth, and the autopilot rightly held level 0 (p≈0.13) — defense in depth against an unknown failure, live. **v0.11.0: consensus gate DEFAULT ON**, armed only when the trade is sound (classification, ≥min train examples/2 labels, AND the calibration half keeps ≥ TESSERA_MIN_GOLD gold — the co-signal never costs cross-validation; below the floors the run is unchanged, so the 24-gold bundled demo stays CV'd while AG-scale gold gets 93.5% coverage with zero flags). v0.11.1: `report`/`serve` default to the dataset's **last-gated target** (runs table) instead of the 0.95 env default — resolution: `--target` flag → explicitly-set env → last run → default; the UI slider and in-server re-gates inherit it.
- **Partner-prep (v0.12.0, 2026-07-20):** the three gaps between "works on our benchmarks" and "runs on a partner's data" closed. **Gold bootstrap mode** (`tessera bootstrap`): cluster-stratified cold-start sample served in the UI for keyboard gold authoring before any model runs (source `bootstrap`; live-verified in the browser incl. undo). **Letter-keyed logprob answers** (`TESSERA_ANSWER_KEY=auto`): lookalike label names (billing_dispute/billing_question) switch the logprob head to lettered options so shared first tokens stop discarding probability mass; existing distinct-word taxonomies and their caches unchanged. **CSV ingest** for items + gold (BOM-tolerant, id-optional, extra columns → metadata) — data as partners actually export it.
- **Dress rehearsal run (v0.12.1, 2026-07-20)** — the §4.3 partner protocol executed end to end on 438 VS Code issues with maintainer labels as held-back truth ([case study](case-study-issue-triage.md)). Three findings: (1) **the annotator-vs-authority gap is the binding risk** — the rehearsal annotator agreed with maintainers only 69.5%, invisible to every internal metric, so session zero with a partner is rubric *calibration* against their own labeled history; (2) **the rubric carries the customer's convention** — writing it into one paragraph took partner-gold coverage 11% → 90.9% (at 82.7% true; the residue is organizational knowledge + thin gold), and the consensus gate is the coverage lever on real data (LLM alone honestly refuses at 0%); (3) **the harness caught a silent serving defect** — Qwen3.5 verbalizes reasoning on long items on a raw llama-server (empty answers, 434/438 items; fix: `--chat-template-kwargs '{"enable_thinking":false}'`), which had also muted the v0.10.0 SMS measurements (erratum in CHANGELOG; corrected SMS numbers are stronger) → shipped the permanent **no-signal guard** (`n_no_signal` per gate + screaming report caveat).
- **Engagement kit (v0.13.0, 2026-07-20):** the case study's lessons as tools. **`tessera rubric-check`** = session zero in one command — diffs a draft rubric against the partner's own labeled history (stratified across their labels; agreement + per-class + disagreement patterns with examples; no-signal surfaced; refuses unknown classes). Self-validated on the rehearsal corpus: rediscovers the question-convention gap (their `question` read at 15% under rubric v1) and caught a rubric-v2 overcorrection the corpus-weighted analysis masked. Plus the **partner one-pager** (docs/partner-one-pager.md) — the sendable pilot pitch. The remaining Phase 0 ask is now purely a human one: close the partner.
- **App-ification (v0.14.0, 2026-07-20, tracker: `appmap.mp`):** `tessera app` = server + UI in one command (most-recently-gated dataset at its own target; first run loads the bundled sample OFFLINE — no surprise model calls; busy ports skipped; `--window` native shell via optional pywebview; live-verified in the browser from a cold empty db). One-file PyInstaller binaries for macOS/Linux/Windows via `.github/workflows/build-app.yml` on version tags (macOS arm64 built + smoked locally: CLI, offline demo, app server). PyPI dist (`tessera-label`) builds clean and the wheel passes the full demo from an isolated venv — upload is a Sam handoff. Native signed/notarized shell (T3) is explicitly gated on the first partner.
